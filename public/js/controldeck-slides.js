...
function updateIM(data){
	$('.chat-content').append(['</br>',data.message].join(''));
...
iosocket.on('connect', function () {
	...
	iosocket.emit("user connected", { username:'test'});
	 iosocket.emit("section update", {section:$('.slide.deck-current').index()}); 
});
...
$(document).bind('deck.change', function(event, from, to) {
   iosocket.emit("section update", {section:to});
});
$('#chat-messsage-form').submit(function(e){
		e.preventDefault();
		iosocket.emit("im", { message:$('#chat-message-input').val()});
		$('.chat-content').append(['</br>',localStorage.getItem('userId'),': ',$('#chat-message-input').val()].join(''));
